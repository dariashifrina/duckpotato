...
app = Flask(__name__)
app.secret_key = "qnwerl;qwlnekraSD:FLAJWE:JRKN@#$190u231:JS"
@app.route("/")
def home():
    username = ""
...
def athletes():
    ...
    if "admin" in session:
        admin = session["admin"]
    return render_template("athletes.html", username = username, admin=admin)
...
def actions():
	...
	return redirect("/")
    return render_template("actions.html", username = username, admin=admin)
...
def editathletes():
   ...
   # return redirect("/")
    ...
    if "username" in r and "password" in r and "email" in r and "first_name" in r and "last_name" in r:
        status = accountManager.register(r["username"],r["password"],r["email"],r["first_name"],r["last_name"],types, admin1)
        if status:
            return redirect("/")
